...
- **store 层迁移至 sqlc** (`app/internal/store/` + `app/cmd/chat/chat.go`)
  - 引入 [sqlc v1.31](https://github.com/sqlc-dev/sqlc) 生成所有 SQL 包装；仓库根新增 [`sqlc.yaml`](./sqlc.yaml)，源查询按表拆到 [`app/internal/store/queries/`](./app/internal/store/queries/)（`user_tenant.sql` / `tenant_info.sql` / `tenant_conv.sql`）
  - 删除三个手写 DAO 接口（`UserTenantStore` / `TenantInfoStore` / `TenantConvStore`）及对应 `*.go` 实现，调用方直接使用 sqlc 生成的 `*Queries` 与行模型
  - 删除 `Conv` 领域类型；时间戳统一为 `int64`（unix epoch seconds，原 `time.Time` 变更为调用方按需 `time.Unix(sec, 0).UTC()` 转换）
  - `*Store` 嵌入 `*Queries`，调用方可直接 `st.CreateUserTenant(...)` / `st.GetTenantConv(...)`；`ConvStatus*` 常量仍手写在 [`model.go`](./app/internal/store/model.go)（sqlc 不为 CHECK 约束生成 Go 常量）
  - `tenant_info.Info` 字段由 `json.RawMessage` 变为 `string`（sqlc 不感知 `json_valid` CHECK）；调用方需传合法 JSON 文本
  - `:exec` 类型不读 `RowsAffected()`，行不存在时不再返回 `sql.ErrNoRows` 包装（用 `:execrows` 或显式 SELECT）
  - 生成产物（`db.go` / `models.go` / `*.sql.go`）提交到 git，`-` 开头注释标明 `Code generated by sqlc. DO NOT EDIT.`，靠 PR review 守住
  - 新增 [`app/internal/store/README.md`](./app/internal/store/README.md) 收口 sqlc 工作流（加表 / 加 query / 注解类型 / 已知限制）
  - 配套测试：`app/internal/store/store_test.go` 18 个用例全部适配新 API；`go test -race ./app/internal/store/` 通过
  - AGENTS.md §2.6 索引同步从「三个 DAO」改为「sqlc 源 + 生成产物 + 手写文件」分层指向
