...
本目录实现四种 LLM 协议到 `app/internal/llm.LLM` 统一接口的适配层：
...
| `DeepSeekChat` | DeepSeek（OpenAI 兼容 Chat Completions） | `github.com/openai/openai-go/v3` |
...
`DeepSeekChat` 是这条原则的"集大成"：DeepSeek 的 Chat Completions 协议与 OpenAI 完全兼容，
所以直接复用 `openai.NewClient(WithBaseURL("https://api.deepseek.com"))`，并直接调用
`openai_chat.go` 中所有 Chat Completions 协议级 free function（`convertHistoryMessage` /
`convertInputMessage` / `convertToolsChat` / `parseChatResponse`），无新增一行 HTTP 解析代码。
...
`DeepSeekChat` 复用 `OpenAIChat` 的协议级转换函数，对外行为与 `OpenAIChat` 一致；差异仅在
...
`DeepSeekChat` 复用 `convertToolsChat`。
